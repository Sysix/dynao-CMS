...
class lang
{
    ...
    {
        self::setAvailableLangs();
        self::setDefault();
        self::setLang(dyn::get('lang'));
        self::setDefaultLangId(dyn::get('langId'));
    }

    static public function setAvailableLangs()
    {
        $sql = new sql();
        $sql->result('SELECT * FROM `' . sql::table('lang') . '` ORDER BY `sort`');
        while ($sql->isNext()) {

            self::$availableLangs[$sql->get('id')] = $sql->get('name');
